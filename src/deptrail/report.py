...
caveat that limits the conclusion. The advisory's reference material — its
installable windows and source URLs — closes the page: the timeline's evidence
sentences already cite the windows inline, and fifty source URLs before the first
actionable row buried it (#93). Coverage alone stays in the head line, because a
CLEAN under a partial feed must not be read as safety.
"""
...
def render_html(report: OrgReport, advisory=None) -> str:
    ...
    out = [
        ...
        f"scanned · worst grade {_grade(report.worst_grade)}"
        + (f" · coverage {escape(advisory.coverage)}" if advisory is not None else "")
        + "</p>",
    ...
    if advisory is not None:
        out.append(
            "<h2>Advisory</h2><p class='sub'>The reference every judgment above "
            f"rests on — coverage {escape(advisory.coverage)}.</p>"
            "<h2>Installable windows (one per judgment)</h2>"
        ...
        out.append(f"<h2>Advisory sources</h2><p class='sub'>{sources}</p>")
