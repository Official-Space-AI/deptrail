...
class TestReportEncoding:
    ...
    def test_html_puts_actions_first_and_advisory_reference_last(self, tmp_path, capsys):
        """#93: 56 source URLs filled the first screen before the first actionable row.

        Coverage stays in the head line — a CLEAN under a partial feed must not be
        read as safety — but the windows table and the source URLs are reference,
        and reference closes the page instead of standing between the banner and
        Rotate.
        """
        main(["demo", "--workdir", str(tmp_path / "demo"), "--format", "html",
              "--output", str(tmp_path / "r.html")])
        html = (tmp_path / "r.html").read_text(encoding="utf-8")
        assert html.index("coverage complete") < html.index("class='banner'")
        assert html.index("<h2>Timeline") < html.index("Installable window")
        assert html.index("<h2>Timeline") < html.index("<h2>Advisory sources")
        assert html.index("<h2>Advisory sources") < html.index("Generated by deptrail")
        assert "https://example.test/demo-advisory" in html
